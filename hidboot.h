...
protected:
        ...
        virtual uint8_t HandleLockingKeys(usb_host_shield::HID* hid, uint8_t key) {
...
class HIDBoot : public usb_host_shield::HID //public USBDeviceConfig, public UsbConfigXtracter
{
...
uint8_t HIDBoot<BOOT_PROTOCOL>::Poll() {
        ...
        if(bPollEnable && ((long)(millis() - qNextPollTime) >= 0L)) {
                ...
                for(int i = 0; i < epMUL(BOOT_PROTOCOL); i++) {
                        ...
                        if(!rcode && read > 2) {
                                if(pRptParser[i])
                                        pRptParser[i]->Parse((usb_host_shield::HID*)this, 0, (uint8_t)read, buf);
#ifdef DEBUG_USB_HOST
